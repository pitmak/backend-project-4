...
function sostavChisla(massivChisel, chislo) {
  const sorted = massivChisel
    .filter((num) => num <= chislo)
    .sort((a, b) => a - b);
  console.log(sorted);
  function getCombinationsForSumm(lastIndex, summ) {
    if (lastIndex < 0 || summ < sorted[0]) {
      return [];
    }

    const lastNumber = sorted[lastIndex];
    console.log('lastNumber: ', lastNumber, 'summ: ', summ);

    if (lastNumber === summ) {
      const res = [
        [lastNumber],
        ...getCombinationsForSumm(lastIndex - 1, summ),
      ];
      console.log('lastNumber === summ: ', res);
      return res;
    }

    const combWithLast = [
      lastNumber,
      ...(getCombinationsForSumm(lastIndex - 1, summ - lastNumber)
        .map((comb) => [...comb, lastNumber])),
    ];
    const combWithoutLast = getCombinationsForSumm(lastIndex - 1, summ);
    return [
      ...combWithLast,
      ...combWithoutLast,
    ];
  ...
  return getCombinationsForSumm(sorted.length - 1, chislo);
}

function sostavChisla1(arr, n) {
